...
Scene5::Scene5(glm::ivec2 posPlayer)
{
	map = NULL;
	player = NULL;
...
void Scene5::initEnemies(vector<pair<glm::ivec2, int>>& enemiesTiles) {
	enemies.clear();
	Wenemies.clear();
...
void Scene5::initPortals(vector< glm::ivec2>& posPortals) {
	Portal port(posPortals[0], glm::ivec2(26, 44), 4);
	Portal port2(posPortals[1], glm::ivec2(0,0), 8);
...
void Scene5::updateEnemies(int deltaTime, glm::ivec2 playerPosition) {
...
void Scene5::updatePlayer(int deltaTime, ShaderProgram& shaderProgram, bool& sceneChanged) {
...
void Scene5::checkIfatackedByEnemy() {
...
bool Scene5::checkBoxOverlapping(const pair<glm::ivec2, glm::ivec2>& box1, const pair<glm::ivec2, glm::ivec2>& box2) {
...
bool Scene5::isOverlapping1D(glm::ivec2 box1, glm::ivec2 box2) {
	return (box1.y >= box2.x && box2.y >= box1.x);
}



void Scene5::checkPickedUpItems() {
...
void Scene5::checkIfRockDamaged(int deltaTime) {
	pair<glm::ivec2, glm::ivec2> boundingBox1 = player->getBoundingBox();
...
void Scene5::checkIfWatterDropDamaged() {
	pair<glm::ivec2, glm::ivec2> boundingBox1 = player->getBoundingBox();
...
void Scene5::checkIfRockHitTheGround(int deltaTime) {
	list<FallingRock*>::iterator it = rockObstacles.begin();
	for (it; it != rockObstacles.end(); ++it) {
...
void Scene5::checkIFplayerNextToFire() {
...
void Scene5::checkIfDamagedByFire() {
	pair<glm::ivec2, glm::ivec2> boundingBox1 = player->getBoundingBox();
	list<Fire*>::iterator it = fireObstacles.begin();
	for (it; it != fireObstacles.end(); ++it) {
		pair<glm::ivec2, glm::ivec2> boundingBox2 = (*it)->getBoundingBox();
...
void Scene5::checkKilledEnemies() {
	int tileSize = map->getTileSize();
...
void Scene5::setPlayerState(PlayerState* playerState) {
	player->setState(playerState);
}

void Scene5::setPlayerPosition(glm::ivec2 pposition) {
