...
Scene::Scene(glm::ivec2 posPlayer)
{
	map = NULL;
	player = NULL;
...
void Scene::initEnemies(vector<pair<glm::ivec2, int>> &enemiesTiles) {
	enemies.clear();
	Wenemies.clear();
...
void Scene::initPortals(vector< glm::ivec2>& posPortals) {
	Portal port(posPortals[0], glm::ivec2(25, 9), 2);
	Portal port2(posPortals[1], glm::ivec2(4,5), 3);
...
void Scene::updateEnemies(int deltaTime, glm::ivec2 playerPosition) {
...
void Scene::updatePlayer(int deltaTime, ShaderProgram& shaderProgram, bool& sceneChanged) {
...
void Scene::checkIfatackedByEnemy() {
...
bool Scene::checkBoxOverlapping(const pair<glm::ivec2, glm::ivec2>& box1, const pair<glm::ivec2, glm::ivec2>& box2) {
...
bool Scene::isOverlapping1D(glm::ivec2 box1, glm::ivec2 box2) {
	return (box1.y >= box2.x && box2.y >= box1.x);
}



void Scene::checkPickedUpItems() {
...
void Scene::checkIfRockDamaged(int deltaTime) {
...
void Scene::checkIfWatterDropDamaged() {
	pair<glm::ivec2, glm::ivec2> boundingBox1 = player->getBoundingBox();
...
void Scene::checkIfRockHitTheGround(int deltaTime) {
	list<FallingRock*>::iterator it = rockObstacles.begin();
	for (it; it != rockObstacles.end(); ++it) {
...
void Scene::checkIFplayerNextToFire() {
...
void Scene::checkIfDamagedByFire() {
	pair<glm::ivec2, glm::ivec2> boundingBox1 = player->getBoundingBox();
	list<Fire*>::iterator it = fireObstacles.begin();
	for (it; it != fireObstacles.end(); ++it) {
		pair<glm::ivec2, glm::ivec2> boundingBox2 = (*it)->getBoundingBox();
		...
		if (leftDir) {
			if (checkBoxOverlapping(EnemyBox, playerBox) && (player->hitting() || player->isUltimate()) && playerX - enemeyX > 0 && !player->playerIsDamaged()) {
				...
				*it = nullptr;
				...
				engine->play2D("sounds/boneDead.wav");
			}
			else ++it;
		}
		else {
...
void Scene::setPlayerState(PlayerState* playerState) {
	player->setState(playerState);
}

void Scene::setPlayerPosition(glm::ivec2 pposition) {
