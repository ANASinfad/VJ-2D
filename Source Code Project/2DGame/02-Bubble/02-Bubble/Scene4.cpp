...
Scene4::Scene4(glm::ivec2 posPlayer)
{
	map = NULL;
	player = NULL;
...
void Scene4::initEnemies(vector<pair<glm::ivec2, int>>& enemiesTiles) {
	enemies.clear();
	Wenemies.clear();
...
void Scene4::initPortals(vector< glm::ivec2>& posPortals) {
	Portal port(posPortals[0], glm::ivec2(16, 63), 2);
	Portal port2(posPortals[1], glm::ivec2(7, 7), 5);
...
void Scene4::updateEnemies(int deltaTime, glm::ivec2 playerPosition) {
...
void Scene4::updatePlayer(int deltaTime, ShaderProgram& shaderProgram, bool& sceneChanged) {
...
void Scene4::checkIfatackedByEnemy() {
...
bool Scene4::checkBoxOverlapping(const pair<glm::ivec2, glm::ivec2>& box1, const pair<glm::ivec2, glm::ivec2>& box2) {
...
bool Scene4::isOverlapping1D(glm::ivec2 box1, glm::ivec2 box2) {
	return (box1.y >= box2.x && box2.y >= box1.x);
}



void Scene4::checkPickedUpItems() {
...
void Scene4::checkIfRockDamaged(int deltaTime) {
...
void Scene4::checkIfWatterDropDamaged() {
	pair<glm::ivec2, glm::ivec2> boundingBox1 = player->getBoundingBox();
...
void Scene4::checkIfRockHitTheGround(int deltaTime) {
	list<FallingRock*>::iterator it = rockObstacles.begin();
	for (it; it != rockObstacles.end(); ++it) {
...
void Scene4::checkIFplayerNextToFire() {
...
void Scene4::checkIfDamagedByFire() {
	pair<glm::ivec2, glm::ivec2> boundingBox1 = player->getBoundingBox();
	list<Fire*>::iterator it = fireObstacles.begin();
	for (it; it != fireObstacles.end(); ++it) {
		pair<glm::ivec2, glm::ivec2> boundingBox2 = (*it)->getBoundingBox();
...
void Scene4::checkKilledEnemies() {
	int tileSize = map->getTileSize();
...
void Scene4::setPlayerState(PlayerState* playerState) {
	player->setState(playerState);
}

void Scene4::setPlayerPosition(glm::ivec2 pposition) {
