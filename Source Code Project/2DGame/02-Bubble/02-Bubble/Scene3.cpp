...
Scene3::Scene3(glm::ivec2 posPlayer)
{
	map = NULL;
	player = NULL;
...
void Scene3::initEnemies(vector<pair<glm::ivec2, int>>& enemiesTiles) {
	enemies.clear();
	Wenemies.clear();
...
void Scene3::initPortals(vector< glm::ivec2>& posPortals) {
	Portal port(posPortals[0], glm::ivec2(121, 3), 1);
	Portal port2(posPortals[1], glm::ivec2(121, 3), 1);
	portals.push_back(port);
	portals.push_back(port2);
}


void Scene3::initPlayer() {
	player = new Player();
...
void Scene3::updateEnemies(int deltaTime, glm::ivec2 playerPosition) {
...
void Scene3::updatePlayer(int deltaTime, ShaderProgram& shaderProgram, bool& sceneChanged) {
...
void Scene3::checkIfatackedByEnemy() {
...
bool Scene3::checkBoxOverlapping(const pair<glm::ivec2, glm::ivec2>& box1, const pair<glm::ivec2, glm::ivec2>& box2) {
...
bool Scene3::isOverlapping1D(glm::ivec2 box1, glm::ivec2 box2) {
	return (box1.y >= box2.x && box2.y >= box1.x);
}



void Scene3::checkPickedUpItems() {
...
void Scene3::checkIfRockDamaged(int deltaTime) {
...
void Scene3::checkIfWatterDropDamaged() {
	pair<glm::ivec2, glm::ivec2> boundingBox1 = player->getBoundingBox();
...
void Scene3::checkIfRockHitTheGround(int deltaTime) {
	list<FallingRock*>::iterator it = rockObstacles.begin();
	for (it; it != rockObstacles.end(); ++it) {
...
void Scene3::checkIFplayerNextToFire() {
...
void Scene3::checkIfDamagedByFire() {
	pair<glm::ivec2, glm::ivec2> boundingBox1 = player->getBoundingBox();
	list<Fire*>::iterator it = fireObstacles.begin();
	for (it; it != fireObstacles.end(); ++it) {
		pair<glm::ivec2, glm::ivec2> boundingBox2 = (*it)->getBoundingBox();
...
void Scene3::checkKilledEnemies() {
	int tileSize = map->getTileSize();
...
void Scene3::setPlayerState(PlayerState* playerState) {
	player->setState(playerState);
}
void Scene3::setPlayerPosition(glm::ivec2 pposition) {
